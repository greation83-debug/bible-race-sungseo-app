...
const formatSaehangulText = (text) => {
    ...
    // 패턴 설명 개선:
    // - (^|[.!?""“”‘’\(\)\[\]]\s*) : 문장 시작, 또는 각종 문장 부호/괄호 뒤(공백 포함)
    // - (\d+) : 절 번호 (1자리 이상 숫자)
    // - (?=[가-힣]) : 뒤에 한글이 오는 경우만 (단위 제외용 lookahead)
    // - (?!(개월|개|년|일|번|차|회|명|층|시|분|초|세|살|권|편|절|장|막)(?![가-힣])) : 
    //   단위가 아닌 경우. (단위 뒤에 다른 한글이 붙으면 단위가 아닌 것으로 간주, 예: '장'은 단위지만 '장인'은 아님)
    const versePattern = /(^|[.!?""“”‘’\(\)\[\]]\s*)(\d+)(?=[가-힣])(?!(개월|개|년|일|번|차|회|명|층|시|분|초|세|살|권|편|절|장|막)(?![가-힣]))/gm;
